...
// A loop over Centrality index
  for (int c_index = 0; c_index < cMax; c_index ++)
  {
    ...
    for (int index = 0; index < etaMax; index++)
    {
        ...
        {
          if ((Centrality_V0A) > c_index*10 - 10 && (Centrality_V0A) < c_index*10)
          {
...
void TreeLRC::Asymmetric()
{
  ...
  int bMax = 10; // bins width for centrality
  // delta = 0 is for bin width = 10
    ...
    for (int index = 0; index < etaMax; index++)
      {
        ...
        if (Centrality_V0A < 80)
        {
          ...
          for(int delta = 1; delta <= bMax; delta++){
            ...
            if(abs(Centrality_V0A - binCenter) <= ((float)delta)/2.0){
              if(jentry < 50)
              {
                std::cout << "Centrality: "+std::to_string(Centrality_V0A)+", c_index = "+std::to_string(c_index)+", binCenter = "+std::to_string(binCenter)+", delta = "+std::to_string(delta)  <<std::endl;
              }
              if(delta == bMax)
              {
                nEvents[index][c_index][0]++;
                nParticles[index][c_index][0] += (double)(N[index]);

                for(int index2 = 0; index2 < etaMax; index2++)
                {
                  nParticlesXY[index][index2][c_index][0] += (double)(N[index]) * (double)(N[index2]);
                }
              }
...
for(int delta = 1; delta < bMax; delta++){
  ...
  // Writing the means and rms to .CSV
  std::ofstream outFile("data_V0A_Asymmetric_bin_width_VertexZ_cut_test.csv");
  outFile << "eta index 1";
  outFile << ",";
  outFile << "eta index 2";
  outFile << ",";
